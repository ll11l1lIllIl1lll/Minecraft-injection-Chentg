...
public class AntiAim extends Module {
    ...
    public void onMotion(final EventMotion eventMotion) {
        if (eventMotion.getEventType() == EventType.PRE && this.sneak.getValueState()) {
            ...
            if (this.shouldsneak) {
                this.mc.getNetHandler().addToSendQueue(new C0BPacketEntityAction(this.mc.thePlayer, 0));
            }
            else {
                this.mc.getNetHandler().addToSendQueue(new C0BPacketEntityAction(this.mc.thePlayer, 1));
            }
        }
        if (eventMotion.getEventType() == EventType.PRE && !this.mc.gameSettings.keyBindUseItem.getIsKeyPressed() && Aura.target == null && Aimbot.target == null) {
