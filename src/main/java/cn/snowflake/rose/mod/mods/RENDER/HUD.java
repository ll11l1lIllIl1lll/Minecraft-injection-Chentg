    ...
    public void on2D(EventRender2D e){
        ...
        if (this.logo.getValueState()){
            if (logomode.isCurrentMode("CSGO")){
                ...
                String text2 = null;
                try {
                    text2 = JReflectUtility.getField(mc.getClass(), ClientLoader.runtimeObfuscationEnabled ? "field_71470_ab": "debugFPS",true).getInt(mc) + " fps | "+" username : "+Client.username +" | " +result +" | " + server;
                ...
                RenderUtil.drawGradientRect(5, posY + 3, 4 + (width / 3), posY + 4,true,
                        rainbow(100),
                        rainbow(1000));
                ...
                RenderUtil.drawRoundedRect(-2,5,Client.instance.fontManager.simpleton30.getStringWidth(logotext) + 1,7 + Client.instance.fontManager.simpleton30.FONT_HEIGHT + 2,2,-1);
                RenderUtil.drawRoundedRect(-2,5,Client.instance.fontManager.simpleton30.getStringWidth(logotext),7 + Client.instance.fontManager.simpleton30.FONT_HEIGHT + 2,2,new Color(25,25,25).getRGB());
