...
public final class GLUProjection {
    ...
    public void updateMatrices(IntBuffer viewport, FloatBuffer modelview, FloatBuffer projection, double widthScale, double heightScale) {
        ...
        double pitch = - fv.y > 0.0 && yaw >= 90.0 && yaw < 270.0 || fv.y > 0.0 && (yaw < 90.0 || yaw >= 270.0) ? Math.toDegrees(Math.atan2(nuv.cross(uv).length(), nuv.dot(uv))) : - Math.toDegrees(Math.atan2(nuv.cross(uv).length(), nuv.dot(uv)));
    ...
    public Projection project(double x, double y, double z, ClampMode clampModeOutside, boolean extrudeInverted) {
            ...
            block16 : {
                block15 : {
                    block14 : {
                        ...
                        if (!outsideFrustum) break block13;
                        opposite = posVec.sub(this.frustumPos).dot(this.viewVec) <= 0.0;
                        ...
                        if (!extrudeInverted) break block14;
                        if (!outsideInvertedFrustum) break block15;
                        if (!outsideInvertedFrustum) break block16;
                    }
                    if (clampModeOutside == ClampMode.NONE) break block16;
                }
                ...
                if (clampModeOutside != ClampMode.ORTHOGONAL) return new Projection(0.0, 0.0, Projection.Type.FAIL);
                if (!GLU.gluProject((float)((float)x), (float)((float)y), (float)((float)z), (FloatBuffer)this.modelview, (FloatBuffer)this.projection, (IntBuffer)this.viewport, (FloatBuffer)this.coords)) return new Projection(0.0, 0.0, Projection.Type.FAIL);
                ...
                if (guiY < 0.0) {
                    ...
                    if (guiY <= this.displayHeight * this.heightScale) return new Projection(guiX, guiY, Projection.Type.OUTSIDE);
        ...
        if (!GLU.gluProject((float)((float)x), (float)((float)y), (float)((float)z), (FloatBuffer)this.modelview, (FloatBuffer)this.projection, (IntBuffer)this.viewport, (FloatBuffer)this.coords)) return new Projection(0.0, 0.0, Projection.Type.FAIL);
        double guiX2 = (double)this.coords.get(0) * this.widthScale;
        double guiY2 = (this.displayHeight - (double)this.coords.get(1)) * this.heightScale;
        return new Projection(guiX2, guiY2, Projection.Type.INSIDE);
    }

    private boolean[] doFrustumCheck(Vector3D[] frustumCorners, Vector3D frustumPos, double x, double y, double z) {
    ...
    private boolean crossPlane(Vector3D[] plane, Vector3D point) {
        Vector3D z = new Vector3D(0.0, 0.0, 0.0);
        Vector3D e0 = plane[1].sub(plane[0]);
        Vector3D e2 = plane[2].sub(plane[0]);
        Vector3D normal = e0.cross(e2).snormalize();
        double D = z.sub(normal).dot(plane[2]);
        double dist = normal.dot(point) + D;
        return dist >= 0.0;
    }

    private Vector3D[] getFrustum(double x, double y, double z, double rotationYaw, double rotationPitch, double fov, double farDistance, double aspectRatio) {
    ...
    private Vector3D getRotationVector(double rotYaw, double rotPitch) {
    ...
    public static class Projection {
        ...
        Projection(double x, double y, Type t) {
            this.x = x;
            this.y = y;
            this.t = t;
        }
    ...
    public static class Line {
        Vector3D sourcePoint = new Vector3D(0.0, 0.0, 0.0);
        Vector3D direction = new Vector3D(0.0, 0.0, 0.0);

        Line(double sx, double sy, double sz, double dx, double dy, double dz) {
